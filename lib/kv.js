...
import { defaultMenu, defaultPartners, defaultDeliveryZones, normalizeMenu } from "./defaults";
...
export async function getOrInitMenu() {
  ...
  // normalizeMenu() repairs any item still in an old/malformed shape
  // (e.g. missing the current `variants` array) so nothing downstream --
  // staff app or public order page -- ever crashes on it. See its comment
  // in lib/defaults.js for details.
  if (Array.isArray(existing) && existing.length > 0) return normalizeMenu(existing);
...
export async function getOrInitDeliveryZones() {
  const existing = await getKey("deliveryZones", null);
  if (Array.isArray(existing) && existing.length > 0) return existing;
  const fresh = defaultDeliveryZones();
  await setKey("deliveryZones", fresh);
  return fresh;
}
